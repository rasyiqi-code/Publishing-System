...
import { SYSTEM_ROLES } from '@repo/core';
...
export const ProjectDetail = ({ project, onBack, userRole, permissions, onUpdateStatus }: ProjectDetailProps) => {
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="relative space-y-1">
                            ...
                            {project.steps.map((step: any, i: number) => {
                                // Logic: Determine if user can act on this step
                                const canAction = !step.isLocked && (
                                    userRole === SYSTEM_ROLES.ADMIN ||
                                    userRole === SYSTEM_ROLES.SUPER_ADMIN ||
                                    // Capability Check (Priority to Permission)
                                    (step.requiredPermission
                                        ? permissions?.[step.requiredPermission] === 'edit'
                                        : userRole === step.roleId // Fallback: Match ID only if no permission key
                                    )
                                );
                    ...
                    <div className="space-y-6">
                        <div className="bg-slate-900 p-8 rounded-[32px] text-white shadow-2xl shadow-slate-200 sticky top-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="p-4 bg-indigo-900/50 rounded-xl border border-indigo-500/30">
                                    <p className="text-[10px] font-bold text-indigo-300 uppercase mb-1">Paket / Layanan</p>
                                    <p className="font-bold text-white text-lg leading-tight">{project.productName || 'N/A'}</p>
                                </div>
                                ...
                                <div className="pt-4">
                                    <button
                                        onClick={handleShare}
                                        className="w-full py-4 bg-indigo-600 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-500 transition-all">
